...
public:
    ...
    void deflect()
    {
        for (int i = 2; i < elements.size(); i++) {
            if (elements[0]->contains(elements[i]->x + 0.5 * elements[i]->w, elements[i]->y - elements[i]->h) == 1 && elements[i]->objectName != "Ball") //Checks if powerup hits paddle
            {
                ...
                if (elements[i]->randNum == 5) {                //done
                    elements[0]->powerupName = "DOUBLE POINTS";
                    elements[1]->score *= 2;
                    //cout << elements[0]->powerupName << endl;
                }

                if (elements[i]->randNum == 10) {               //done
...

            {
                ...
                if (elements[i]->objectName != "Ball" && elements[j]->objectName == "Ball") {
                    ...
                    if (topOfBall || bottomOfBall && elements[i]->isPowerup == false) {
                        ...
                        if (elements[i]->randNum % 5  == 0) //special nums are: 0, 3, 6, 9, 12, 18
                        {
			    elements[i]->r = 1.0;	//here is where we have to check the randNum to decide which powerup has which color..
		            elements[i]->g = 0.0;
			    elements[i]->b = 0.0;
    ...
    void createHUD()
    {
	 ...
	 glColor3f(0.0, 0.0, 0.0); //Powerup bar
        glBegin(GL_POLYGON);
